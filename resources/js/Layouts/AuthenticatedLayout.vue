...
<template>
    <div class="flex h-screen bg-[#F8FAFC] overflow-hidden font-sans text-slate-900">
        ...
        >
            <div class="h-16 flex items-center px-6 border-b border-slate-100 shrink-0 bg-white">
                <Link :href="logoRoute" class="flex items-center gap-3 group overflow-hidden">
                    ...
                    <div class="flex flex-col">
                        ...
                        <span class="text-[9px] font-medium text-indigo-600 uppercase tracking-wider leading-none">CPNS Academy</span>
                    </div>
                </Link>
            </div>
            ...
            <nav ref="sidebarNav" class="flex-1 overflow-y-auto py-4 px-4 space-y-0.5 custom-scrollbar">
                
                <div v-if="isAdmin" class="space-y-0.5">
                    <p class="text-[10px] uppercase font-semibold text-rose-500 px-4 mb-1 mt-2 tracking-[0.2em]">Admin Control</p>
                    
                    <Link :href="route('admin.dashboard')" :class="[route().current('admin.dashboard') ? 'bg-rose-50 text-rose-600 active-link shadow-sm' : 'text-slate-500 hover:bg-rose-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 flex-shrink-0 text-rose-500"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z" /></svg>
                        <span class="text-[11px] uppercase tracking-wider">Dashboard Admin</span>
                    </Link>

                    <Link :href="route('admin.users.index')" :class="[route().current('admin.users.*') ? 'bg-rose-50 text-rose-600 active-link shadow-sm' : 'text-slate-500 hover:bg-rose-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                    ...
                    <Link :href="route('admin.tryouts.index')" :class="[(route().current('admin.tryouts.*') && !route().current('admin.tryouts.questions.*')) || (route().current('admin.tryouts.questions.*') && activeTryoutType !== 'adidaya') ? 'bg-rose-50 text-rose-600 active-link shadow-sm' : 'text-slate-500 hover:bg-rose-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                        ...
                        <span class="text-[11px] uppercase tracking-wider">Kelola Tryout</span>
                    </Link>

                    <Link :href="route('admin.affiliate.leaderboard')" :class="[route().current('admin.affiliate.leaderboard') ? 'bg-rose-50 text-rose-600 active-link shadow-sm' : 'text-slate-500 hover:bg-rose-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                        ...
                        <span class="text-[11px] uppercase tracking-wider">Ranking Affiliate</span>
                    </Link>
                    ...
                    <Link :href="route('admin.adidaya-manage.index')" :class="[route().current('admin.adidaya-manage.*') || (route().current('admin.tryouts.questions.*') && activeTryoutType === 'adidaya') ? 'bg-rose-50 text-rose-600 active-link shadow-sm' : 'text-slate-500 hover:bg-rose-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                        ...
                        <span class="text-[11px] uppercase tracking-wider">Adidaya Manager</span>
                    </Link>

                    <Link :href="route('admin.tryout-akbar.index')" :class="[route().current('admin.tryout-akbar.*') ? 'bg-rose-50 text-rose-600 active-link shadow-sm' : 'text-slate-500 hover:bg-rose-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                        ...
                        <span class="text-[11px] uppercase tracking-wider">Event Akbar</span>
                    </Link>

                    <Link :href="route('admin.transactions.index')" :class="[route().current('admin.transactions.*') ? 'bg-rose-50 text-rose-600 active-link shadow-sm' : 'text-slate-500 hover:bg-rose-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                        ...
                        <span class="text-[11px] uppercase tracking-wider">Data Transaksi</span>
                    </Link>

                    <p class="text-[10px] uppercase font-semibold text-rose-500 px-4 mb-1 mt-4 tracking-[0.2em]">Afiliasi System</p>
                    
                    <Link :href="route('admin.affiliate.withdrawals')" :class="[route().current('admin.affiliate.withdrawals') ? 'bg-rose-50 text-rose-600 active-link shadow-sm' : 'text-slate-500 hover:bg-rose-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 flex-shrink-0 text-green-500"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v12m-3-2.818.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
                        <span class="text-[11px] uppercase tracking-wider">Permintaan WD</span>
                    </Link>

                    <Link :href="route('admin.affiliate.transactions')" :class="[route().current('admin.affiliate.transactions') ? 'bg-rose-50 text-rose-600 active-link shadow-sm' : 'text-slate-500 hover:bg-rose-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 flex-shrink-0 text-cyan-500"><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z" /></svg>
                        <span class="text-[11px] uppercase tracking-wider">Data Afiliasi</span>
                    </Link>
                </div>

                <div v-if="isUser && !isAdmin" class="space-y-0.5">
                    <p class="text-[10px] uppercase font-semibold text-slate-400 px-4 mb-1 mt-2 tracking-[0.2em]">Menu Navigasi</p>
                    
                    <Link :href="route('dashboard')" :class="[route().current('dashboard') ? 'bg-slate-900 text-white shadow-xl active-link' : 'text-slate-500 hover:bg-slate-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                    ...
                    <Link :href="route('tryout.index')" :class="[(route().current('tryout.index') || route().current('tryout.show') || route().current('tryout.register') || route().current('tryout.my') || (route().current('tryout.wait') && activeTryoutType !== 'adidaya')) ? 'bg-slate-900 text-white shadow-xl active-link' : 'text-slate-500 hover:bg-slate-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                        ...
                        <span class="text-[11px] uppercase tracking-wider">Katalog Tryout</span>
                    </Link>

                    <Link :href="route('tryout-akbar.index')" :class="[route().current('tryout-akbar.*') ? 'bg-slate-900 text-white shadow-xl active-link' : 'text-slate-500 hover:bg-slate-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                    ...
                    <Link :href="route('tryout.adidaya')" :class="[(route().current('tryout.adidaya') || (route().current('tryout.wait') && activeTryoutType === 'adidaya')) ? 'bg-slate-900 text-white shadow-xl active-link' : 'text-slate-500 hover:bg-slate-100', 'flex items-center justify-between py-2 px-4 rounded-xl font-medium transition-all group relative']">
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-[11px] uppercase tracking-wider">Nusantara Adidaya</span>
                        </div>
                    ...
                    <Link :href="route('membership.index')" :class="[route().current('membership.*') ? 'bg-slate-900 text-white shadow-xl active-link' : 'text-slate-500 hover:bg-slate-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                        ...
                        <span class="text-[11px] uppercase tracking-wider">Membership</span>
                    </Link>

                    <Link :href="route('affiliate.index')" :class="[route().current('affiliate.*') ? 'bg-slate-900 text-white shadow-xl active-link' : 'text-slate-500 hover:bg-slate-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                        ...
                        <span class="text-[11px] uppercase tracking-wider">Afiliasi</span>
                    </Link>

                    <Link :href="route('tryout.history')" :class="[(route().current('tryout.history') || route().current('tryout.history.*')) ? 'bg-slate-900 text-white shadow-xl active-link' : 'text-slate-500 hover:bg-slate-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                        ...
                        <span class="text-[11px] uppercase tracking-wider">Riwayat Tryout</span>
                    </Link>

                    <Link :href="route('wallet.index')" :class="[(route().current('wallet.index') || route().current('wallet.*')) ? 'bg-slate-900 text-white shadow-xl active-link' : 'text-slate-500 hover:bg-slate-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                        ...
                        <span class="text-[11px] uppercase tracking-wider">Dompet Saya</span>
                    </Link>

                    <Link :href="route('profile.edit')" :class="[(route().current('profile.edit') || route().current('profile.*')) ? 'bg-slate-900 text-white shadow-xl active-link' : 'text-slate-500 hover:bg-slate-50', 'flex items-center gap-2 py-2 px-4 rounded-xl font-medium transition-all group relative']">
                        ...
                        <span class="text-[11px] uppercase tracking-wider">Profil Saya</span>
                    </Link>
                </div>
            </nav>

            <div class="p-4 border-t border-slate-100 bg-slate-50/50">
                <Link :href="route('logout')" method="post" as="button" class="w-full flex items-center justify-center gap-2 py-2 px-4 text-rose-500 hover:bg-rose-50 rounded-xl transition font-medium text-[11px] uppercase tracking-wider active:scale-95">
