...
const form = useForm({
    ...
    published_at: '',
    started_at: '',
    is_paid: true, // Default true untuk Adidaya karena biasanya premium
    price: 0,
});
...
const deleteTryout = (id) => {
    if (confirm('Hapus paket adidaya ini?')) {
        router.delete(route('admin.adidaya-manage.destroy', id));
    }
};
    ...
    <AuthenticatedLayout>
        <div class="max-w-6xl mx-auto mt-8 px-4 sm:px-0">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
                <div>
                    <h2 class="font-black text-3xl text-gray-900 tracking-tighter uppercase italic">Paket Adidaya</h2>
                    <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Eksklusif Premium & Manajemen Konten</p>
                </div>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            <tr v-if="tryouts.data.length === 0">
                                <td colspan="4" class="px-8 py-12 text-center text-gray-400">
                                    <p class="text-xs font-bold uppercase tracking-widest">Belum ada paket adidaya ditemukan</p>
                                </td>
                            </tr>
                ...
                <div class="relative bg-white w-full max-w-xl rounded-[3rem] p-10 shadow-2xl animate-in zoom-in-95 duration-200 overflow-y-auto max-h-[90vh] no-scrollbar">
                    ...
                    <form @submit.prevent="submit" class="space-y-5">
                        <div class="space-y-1">
                            ...
                            <input v-model="form.title" type="text" class="w-full border-gray-100 bg-gray-50 rounded-2xl p-4 focus:ring-indigo-500 font-bold text-sm" placeholder="Contoh: Adidaya Premium #1" required />
                        </div>
                        ...
                        <div class="flex gap-4 pt-6">
                            ...
                            <button type="submit" :disabled="form.processing" class="flex-[2] py-4 bg-gray-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl hover:bg-indigo-600 transition active:scale-95">
                                {{ isEdit ? 'Simpan Perbarui' : 'Buat Paket Adidaya' }}
                            </button>
                        </div>
                    </form>
                </div>
